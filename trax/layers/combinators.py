...
class Serial(base.Layer):
  ...
  def forward_with_state(self, xs, weights, state, rng):
    ...
    for layer, w, s, rng in zip(self.sublayers, weights, state, rngs):
      ...
      outputs, s = layer.pure_fn(inputs, w, s, rng, cache=True)
  ...
  def new_weights_and_state(self, input_signature):
    ...
    for sublayer in self.sublayers:
      ...
      weights_or_empty, state = sublayer.init(inputs, cache=True)
...
class Parallel(base.Layer):
  ...
  def forward_with_state(self, inputs, weights, state, rng):
    ...
    for layer, x, w, s, r in zip(layers, sublayer_inputs, weights, state, rngs):
      ...
      sub_outputs, sub_state = layer.pure_fn(x, w, s, r, cache=True)
      if layer.n_out == 1:
  ...
  def new_weights_and_state(self, input_signature):
    ...
    inits = [layer.init(signature, cache=True)
             for layer, signature
...
class Scan(base.Layer):
  ...
  def forward_with_state(self, inputs, weights, state, rng):
    ...
    def scannable_fn(x, carry_and_state):  # pylint: disable=invalid-name
      ...
      res, new_state = self.sublayer.pure_fn(
          x_and_carry, weights, state, rng, cache=True)
      if n_carry > 0:
  ...
  def new_weights_and_state(self, input_signature):
    ...
    return self.sublayer.init(layer_signature, cache=True)
...
class Cache(base.Layer):
  ...
  def new_weights_and_state(self, input_signature):
    weights, layer_state = self.sublayer.init(input_signature, cache=True)
